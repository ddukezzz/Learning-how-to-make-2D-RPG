...
public class Inventory_Storage : Inventory_Base
{
    ...
    public void ConsumedMaterials(Inventory_Item itemToCraft)
    {
        foreach (var requiredItem in itemToCraft.itemData.craftRecipe)
        {
            ...
            if (amountToConsume > 0) amountToConsume -= ConsumedMaterialsAmount(itemList, requiredItem);
            
            if (amountToConsume > 0) amountToConsume -= ConsumedMaterialsAmount(materialStash, requiredItem);
        }
    }
    ...
    private int ConsumedMaterialsAmount(List<Inventory_Item> itemList, Inventory_Item neededItem)
    {
        ...
        foreach (var item in itemList)
        {
            ...
            int removedAmount = Mathf.Min(item.stackSize, amountNeeded);
            item.stackSize -= removedAmount;
            consumedAmount += removedAmount;
            
            if (item.stackSize <= 0) itemList.Remove(item);
    ...
    public bool HasEnoughMaterial(Inventory_Item itemToCraft)
    {
        ...
        {
            if (GetAvailableAmountOf(requiredMaterial.itemData) < requiredMaterial.stackSize) return false;
        }
    ...
    public int GetAvailableAmountOf(ItemDataSO requiredItem)
    {
        ...
        {
            if (item.itemData == requiredItem) amount = amount + item.stackSize;
        }
